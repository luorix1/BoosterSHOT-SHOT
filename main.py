...
from multiview_detector.models.spatialshot import SpatialSHOT
from multiview_detector.models.channelshot import ChannelSHOT
from multiview_detector.models.self_res_dpersp_trans_detector import (
    SRDPerspTransDetector,
)
...
def main(args):
    ...
    elif args.variant == "ChannelSHOT":
        model = ChannelSHOT(train_set, args.arch, args.depth_scales, args.topk)
    elif args.variant == "SpatialSHOT":
        model = SpatialSHOT(train_set, args.arch, args.depth_scales, args.topk)
    elif args.variant == "self":
        model = SRDPerspTransDetector(
            train_set, args.arch, args.depth_scales, args.use_GN
        )
    elif args.variant == "custom":
        model = RDPerspTransDetector(
            train_set,
            args.arch,
            args.depth_scales,
            args.use_local,
    ...
    if args.scheduler == "OneCycleLR":
        ...
        scheduler = torch.optim.lr_scheduler.CyclicLR(
            ...
            step_size_up=int(0.5 * len(train_loader) * args.epochs),
        )
    else:
    ...
    if logdir is None:
        logdir = (
            f"logs/{args.dataset}_frame/{args.variant}/{args.loginfo}"
            + datetime.datetime.today().strftime("%Y-%m-%d_%H-%M-%S")
            if not args.resume
            else f"logs/{args.dataset}_frame/{args.variant}/{args.resume}"
        )
    ...
    if args.variant == "default" or "custom" or "self":
        trainer = DPerspectiveTrainer(
            model, criterion, logdir, denormalize, args, args.cls_thres, args.alpha
        )
    else:
        trainer = PerspectiveTrainer(
            model, criterion, logdir, denormalize, args.cls_thres, args.alpha
        )
    ...
    if args.resume is None:
        # print('Testing...')
        trainer.test(
            test_loader, os.path.join(logdir, "test.txt"), train_set.gt_fpath, False
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--arch", type=str, default="resnet18", choices=["vgg11", "resnet18"]
    )
    parser.add_argument(
        "-d",
        "--dataset",
    ...
    parser.add_argument(
        "--scheduler",
        default="OneCycleLR",
        choices=["OneCycleLR", "MultiStepLR", "CyclicLR"],
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.001,
        help="optional minimum learning rate for CyclicLR (default: 0.001)",
    )
    ...
    parser.add_argument(
        "--seed", type=int, default=1, help="random seed (default: None)"
    )
