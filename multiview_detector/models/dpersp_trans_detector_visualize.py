...
class DPerspTransDetector(nn.Module):
    ...
    def warp_perspective(self, img_feature_all):
        ...
        for j in range(6):
            ...
            plt.imsave(os.path.expanduser(
                f'~/code/jupyter/dua/data/da/feat-f{j}.png'), feat.cpu().numpy(), cmap='Oranges')
            for i in range(5):
                plt.imsave(os.path.expanduser(
        ...
        for j in range(6):
            ...
            plt.imsave(os.path.expanduser(
                f'~/code/jupyter/dua/data/da/warped-feat-f{j}.png'), t.cpu().numpy())
            t = torch.norm(warped_feat_no_conv[j, ...].detach(), dim=0)
            plt.imsave(os.path.expanduser(
                f'~/code/jupyter/dua/data/da/warped-feat-no-conv-f{j}.png'), t.cpu().numpy())
        t = torch.norm(warped_feat.view(
            [-1, *self.reducedgrid_shape]).detach(), dim=0)
        plt.imsave(os.path.expanduser(
            f'~/code/jupyter/dua/data/da/warped-feat-all.png'), t.cpu().numpy())
        import pdb
        pdb.set_trace()
        return warped_feat
    ...
    def forward(self, imgs, imgs_gt=None, map_gt=None, alpha=0, visualize=False):
        ...
        for i in range(imgs.shape[1]):
            t = Image.fromarray(img[i].cpu().numpy().astype(
                'uint8').transpose([1, 2, 0]))
            t = t.resize((480, 270))
            t.save(os.path.expanduser(
                f'~/code/jupyter/dua/data/da/image-f{i}.png'))
        ...
        for i in range(5):
            ...
            tt.save(os.path.expanduser(
                f'~/code/jupyter/dua/data/da/image{i}.png'))

        import matplotlib.pyplot as plt
        plt.imsave(os.path.expanduser('~/code/jupyter/dua/data/da/map.png'),
                   self.trans_map(map_gt)[0, 0].cpu().numpy())
        plt.imsave(os.path.expanduser('~/code/jupyter/dua/data/da/imgs_gt_0.png'),
                   self.trans_img(imgs_gt)[0, 0].cpu().numpy())
