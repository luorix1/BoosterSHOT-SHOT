    ...
    def forward(self, imgs):
        ...
        for i in range(imgs.shape[1]):
            ...
            t.save(os.path.expanduser(
                f'~/code/jupyter/dua/data/per/image-f{i}.png'))
        ...
        for cam in range(self.num_cam):
            ...
            img_feature = F.interpolate(
                img_feature, self.upsample_shape, mode='bilinear')
            ...
            if visualize:
                plt.imsave(os.path.expanduser(f'~/code/jupyter/dua/data/per/feat-f{cam}-per.png'),
                           torch.norm(img_feature[0].detach(), dim=0).cpu().numpy(), cmap='Oranges')
                plt.imsave(os.path.expanduser(f'~/code/jupyter/dua/data/per/warped-feat-f{cam}-per.png'),
                           torch.norm(world_feature[0].detach(), dim=0).cpu().numpy())
            world_features.append(world_feature.to('cuda:0'))

        if visualize:
            w = torch.cat(world_features, dim=1)
            plt.imsave(os.path.expanduser(f'~/code/jupyter/dua/data/per/warped-feat-all-per.png'),
                       torch.norm(w[0].detach(), dim=0).cpu().numpy())
        world_features = torch.cat(
            world_features + [self.coord_map.repeat([B, 1, 1, 1]).to('cuda:0')], dim=1)
        ...
        if visualize:
            plt.imsave(os.path.expanduser(
                f'~/code/jupyter/dua/data/per/map_result-per.png'), map_result[0, 0].detach().cpu().numpy())
        import pdb
        pdb.set_trace()
