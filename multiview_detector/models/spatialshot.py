...
class SpatialSHOT(nn.Module):
    def __init__(self, dataset, arch="resnet18", depth_scales=4, topk=None):
        ...
        for i in range(self.depth_scales):
            proj = (
                torch.stack(proj_mats[i]).float()[None].repeat([batch_size, 1, 1, 1])
            )  # B
            all_proj = 0
            proj_mats[i] = nn.Parameter(proj.view([-1, 3, 3]), requires_grad=False).to(
                "cuda:0"
            )
        ...
        self.img_classifier = nn.Sequential(
            nn.Conv2d(out_channel, 64, 1), nn.ReLU(), nn.Conv2d(64, 2, 1, bias=False)
        ).to("cuda:0")

        # The feat_down network
        self.feat_down = nn.Sequential(
            nn.Conv2d(out_channel, 128, 3, padding=2, dilation=2),
            nn.ReLU(),
        ...
        self.feat_before_concat = nn.Conv2d(
            out_channel, out_channel, 3, groups=self.depth_scales, padding=1
        ).to("cuda:0")
        ...
        self.trans_img = nn.Sequential(
            nn.AdaptiveAvgPool2d(self.upsample_shape),
            nn.Conv2d(2, 2, 21, padding=10, bias=False),
        )
        ...
        self.trans_map = nn.Sequential(
            nn.AdaptiveAvgPool2d(self.reducedgrid_shape),
            nn.Conv2d(2, 2, 41, padding=20, bias=False),
        )
    ...
    def forward(self, imgs, imgs_gt=None, map_gt=None, alpha=0, visualize=False):
        ...
        img_feature_all = F.interpolate(
            img_feature_all, self.upsample_shape, mode="bilinear"
        ).to("cuda:0")
        ...
        world_features = torch.cat(
            [
                world_features.view([B, -1, *self.reducedgrid_shape]),
                self.coord_map.repeat([B, 1, 1, 1]),
            ],
            dim=1,
        )
    ...
    def get_imgcoord2worldgrid_matrices(
        ...
        for cam in range(self.num_cam):
            for i in range(self.depth_scales):
                ...
                extrinsic_matrices[cam][:, 3] = (
                    extrinsic_matrices[cam][:, 3]
                    + extrinsic_matrices[cam][:, 2] * i * depth_margin
                )
                worldcoord2imgcoord_mat = intrinsic_matrices[cam] @ np.delete(
                    extrinsic_matrices[cam], 2, 1
                )
                worldgrid2imgcoord_mat = (
                    worldcoord2imgcoord_mat @ worldgrid2worldcoord_mat
                )
    ...
    def create_coord_map(self, img_size, with_r=False):
        ...
        if with_r:
            rr = torch.sqrt(torch.pow(grid_x, 2) + torch.pow(grid_y, 2)).view(
                [1, 1, H, W]
            )
