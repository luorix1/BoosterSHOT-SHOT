...
class DPerspectiveTrainer(BaseTrainer):
    def __init__(
        self, model, criterion, logdir, denormalize, args, cls_thres=0.4, alpha=1.0
    ):
        super(BaseTrainer, self).__init__()
        self.model = model
    ...
    def train(
        self, epoch, data_loader, optimizer, log_interval=100, cyclic_scheduler=None
    ):
        self.model.train()
        losses = 0
    ...
    def test(self, data_loader, res_fpath=None, gt_fpath=None, visualize=False):
        ...
        for batch_idx, (data, map_gt, imgs_gt, frame) in enumerate(data_loader):
            ...
            for img_res, img_gt in zip(imgs_res, imgs_gt):
                loss += self.criterion(
                    img_res, img_gt.to(img_res.device), data_loader.dataset.img_kernel
                )
            loss = (
                self.criterion(
        ...
        if visualize:
            ...
            subplt1.imshow(
                self.criterion._traget_transform(
        ...
        if res_fpath is not None:
            ...
            np.savetxt(
                os.path.abspath(os.path.dirname(res_fpath)) + "/all_res.txt",
                all_res_list.numpy(),
                "%.8f",
            )
            res_list = []
            for frame in np.unique(all_res_list[:, 0]):
                ...
                res_list.append(
                    torch.cat(
                        [torch.ones([count, 1]) * frame, positions[ids[:count], :]],
                        dim=1,
                    )
                )
            res_list = (
            ...
            if not self.args.no_matlab:
                ...
                print(
                    "moda: {:.1f}%, modp: {:.1f}%, precision: {:.1f}%, recall: {:.1f}%".format(
                        moda, modp, precision, recall
                    )
                )

        print(
            "Test, Loss: {:.6f}, Precision: {:.1f}%, Recall: {:.1f}, \tTime: {:.3f}".format(
                losses / (len(data_loader) + 1),
                precision_s.avg * 100,
                recall_s.avg * 100,
                t_epoch,
            )
...
class BBOXTrainer(BaseTrainer):
    ...
    def train(
        ...
        for batch_idx, (data, target, _) in enumerate(data_loader):
            ...
            if (batch_idx + 1) % log_interval == 0:
                ...
                t_epoch = t1 - t0
                print(
                    "Train Epoch: {}, Batch:{}, \tLoss: {:.6f}, Prec: {:.1f}%, Time: {:.3f}".format(
                        epoch,
                        (batch_idx + 1),
                        losses / (batch_idx + 1),
                        100.0 * correct / (correct + miss),
                        t_epoch,
                    )
                )
    ...
    def test(self, test_loader, log_interval=100, res_fpath=None):
        ...
        all_res_list = []
        t0 = time.time()
        for batch_idx, (data, target, (frame, pid, grid_x, grid_y)) in enumerate(
            test_loader
        ):
            data, target = data.cuda(), target.cuda()
            with torch.no_grad():
            ...
            if res_fpath is not None:
                ...
                all_res_list.append(
                    torch.stack(
                        [
                            frame[indices].float(),
                            grid_x[indices].float(),
                            grid_y[indices].float(),
                            output[indices, 1].cpu(),
                        ],
                        dim=1,
                    )
                )
            if (batch_idx + 1) % log_interval == 0:
                ...
                t_epoch = t1 - t0
                print(
                    "Test Batch:{}, \tLoss: {:.6f}, Prec: {:.1f}%, Time: {:.3f}".format(
                        (batch_idx + 1),
                        losses / (batch_idx + 1),
                        100.0 * correct / (correct + miss),
                        t_epoch,
